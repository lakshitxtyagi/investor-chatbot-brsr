...
# Matches taxonomy placeholder values like 'StockExchange1', 'CSRProjects2',
# 'AppealOrRevisiondomain1' — camelCase identifiers ending with a digit.
# Real data values (false, No, Holding, 0, 1.5) never match this pattern.
_TAXONOMY_VALUE_RE = re.compile(r"^[A-Za-z]{4,}\d+$")


def is_domain_row(element: str, value: str = "") -> bool:
    """True for XBRL taxonomy domain/member placeholder rows (not real data).

    Requires BOTH conditions to be true:
      1. Element name ends in 'domain' or 'member' (case-insensitive)
      2. Value looks like a taxonomy placeholder (camelCase + digit suffix),
         OR no value is provided.

    This correctly preserves real Yes/No fields like:
      WhetherInformationAvailableInPublicDomain → false  (KEPT)
      ResultsCommunicatedInPublicDomain         → false  (KEPT)
    While still dropping:
      StockExchangeDomain                       → StockExchange1  (DROPPED)
      CSRProjectsDomain                         → CSRProjects1    (DROPPED)
    """
    lower = element.lower()
    if not (lower.endswith("domain") or lower.endswith("member")):
        return False
    if value and not _TAXONOMY_VALUE_RE.match(value.strip()):
        return False
    return True
...
def filter_df(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    Row classification:
        Domain rows  → element ends in 'domain'/'member' AND value is a
                       taxonomy placeholder (e.g. StockExchange1) → dropped
        Narrative    → element contains 'ExplanatoryTextBlock'    → narrative_df
        Everything else (numeric, boolean, categorical, including
        fields like WhetherInformationAvailableInPublicDomain)    → scalar_df
    """
    mask_domain = df.apply(lambda r: is_domain_row(r["element"], r["value"]), axis=1)
...
def write_jsonl(
    chunks: list[dict], out_dir: Path, filename: str = "chunks.jsonl"
) -> Path:
